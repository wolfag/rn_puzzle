...
export default class Board extends React.PureComponent {
  ...
  handleTouchMove(square, index, { top, left }) {
    ...
    const clampedTop = clamp(
      top,
      move === DIRECTION.UP ? -distance : 0,
      move === DIRECTION.DOWN ? distance : 0,
    );

    const clampedLeft = clamp(
      left,
      move === DIRECTION.LEFT ? -distance : 0,
      move === DIRECTION.RIGHT ? distance : 0,
    );
  ...
  handleTouchEnd(square, index, { top, left }) {
    ...
    if (
      (move === DIRECTION.UP && top < -itemSize / 2)
      || (move === DIRECTION.DOWN && top > itemSize / 2)
      || (move === DIRECTION.LEFT && left < -itemSize / 2)
      || (move === DIRECTION.RIGHT && left > itemSize / 2)
    ) {
      onMoveSquare(square);
    } else {
